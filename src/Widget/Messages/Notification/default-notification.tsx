...
export default function DefaultNotification({ poll, sendResponse }: { poll: any; sendResponse: Function }) {
   ...
   const h1 = (text: string) => <h1 className="text-xs font-semibold  text-neutral-800 dark:text-neutral-100">{text}</h1>;
   const ComButton = () => (
      <Button
   ...
   return (
      <div className="flex flex-col items-start  p-4  ">
         ...
         {/* <p className="text-xs font-semibold  text-neutral-800 dark:text-neutral-100">{poll.poll_data.title}</p>
            {poll.poll_data.subtitle && <p className="text-xs text-neutral-500 dark:text-neutral-400">{poll.poll_data.subtitle}</p>} */}
      </div>
   );
}
